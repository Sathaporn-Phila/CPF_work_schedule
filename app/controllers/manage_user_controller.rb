    ...
    def set_plan(plan_all)
        plan_all.each do |item|
            ...
            if  DateTime.now() < new_shift_time#plan ก่อนเวลาเข้างาน
                @new_plan_time = SchedulePlantime.create(shift_code:shifttime.code_name,time_in:new_shift_time,time_out:new_shift_time+9.hours,ot_time:@ot_time)
            else#plan หลังเข้างาน
                @new_plan_time = SchedulePlantime.create(shift_code:shifttime.code_name,time_in:new_shift_time+1.days,time_out:new_shift_time+1.days+9.hours,ot_time:@ot_time)
            end
